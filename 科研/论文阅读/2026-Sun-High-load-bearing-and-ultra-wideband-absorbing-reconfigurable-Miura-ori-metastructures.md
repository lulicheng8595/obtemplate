...
- Zotero item key：ZMCZ4L4A
- 注意：先前周报中写成“Penglin He et al.”不准确；ScienceDirect、作者主页和 RSC forward links 显示首作者为 Chengtao Sun。

## 阅读状态与边界

当前 Zotero 条目尚无 PDF 附件，ScienceDirect 自动抓取 PDF 返回 HTTP 403。因此，本文正式精读需要基于你打开的全文 PDF、截图或后续添加到 Zotero 的附件进行。下面暂时只保留已核验的题录和我们已经确认的初步判断；图表、实验参数和性能机制将在共读后再写入。
